...
echo "<ul>";
 for ($i=0; $i < count($data); $i++) {
	if ($data[$i]['PARENT_ID'] == 0) {
		...
		echo "<ul>";
			for ($i=0; $i < count($data); $i++) {
				if ($data[0]['ID'] == $data[$i]['PARENT_ID']) {
				echo "<li>".$data[$i]['NAME'];
				echo "<ul>";
					for ($i=0; $i < count($data); $i++) {
						if ($data[4]['ID'] == $data[$i]['PARENT_ID']) {
						echo "<li>".$data[$i]['NAME'];
						echo "<ul>";
								for ($i=0; $i < count($data); $i++) {
								if ($data[8]['ID'] == $data[$i]['PARENT_ID']) {
								echo "<li>".$data[$i]['NAME'];
									} 
								}
...
for ($i=1; $i < count($data); $i++) {
	if ($data[$i]['PARENT_ID'] == 0) {
		...
		echo "<ul>";
			for ($i=0; $i < count($data); $i++) {
				if ($data[1]['ID'] == $data[$i]['PARENT_ID']) {
				echo "<li>".$data[$i]['NAME'];
				echo "<ul>";
					for ($i=0; $i < count($data); $i++) {
						if ($data[16]['ID'] == $data[$i]['PARENT_ID']) {
						echo "<li>".$data[$i]['NAME'];
						echo "<ul>";
								for ($i=0; $i < count($data); $i++) {
								if ($data[22]['ID'] == $data[$i]['PARENT_ID']) {
								echo "<li>".$data[$i]['NAME'];
									} 
								}
...
for ($i=2; $i < count($data); $i++) {
	if ($data[$i]['PARENT_ID'] == 0) {
		...
		echo "<ul>";
			for ($i=0; $i < count($data); $i++) {
				if ($data[2]['ID'] == $data[$i]['PARENT_ID']) {
				echo "<li>".$data[$i]['NAME'];
				echo "<ul>";
					for ($i=0; $i < count($data); $i++) {
						if ($data[18]['ID'] == $data[$i]['PARENT_ID']) {
						echo "<li>".$data[$i]['NAME'];
						echo "<ul>";
								for ($i=0; $i < count($data); $i++) {
								if ($data[28]['ID'] == $data[$i]['PARENT_ID']) {
								echo "<li>".$data[$i]['NAME'];
									} 
								}
...
for ($i=3; $i < count($data); $i++) {
	if ($data[$i]['PARENT_ID'] == 0) {
		...
		echo "<ul>";
			for ($i=0; $i < count($data); $i++) {
				if ($data[3]['ID'] == $data[$i]['PARENT_ID']) {
				echo "<li>".$data[$i]['NAME'];
				echo "<ul>";
					for ($i=0; $i < count($data); $i++) {
						if ($data[20]['ID'] == $data[$i]['PARENT_ID']) {
						echo "<li>".$data[$i]['NAME'];
